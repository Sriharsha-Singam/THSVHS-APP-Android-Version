...
//            String
    ...
    public void checkAccessCode(){
        ...
        alert.setMessage("And Create Your Very Own Account");
...
//                Toast.makeText(Login.this, input.getText(),
//                        Toast.LENGTH_SHORT).show();
                Firebase.setAndroidContext(Login.this);
                final Firebase ref = new Firebase("//fir-learn-4991c.firebaseio.com/AccessCode");

                ValueEventListener valueEventListener=new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        System.out.println("Access Code Snapshot:"+ dataSnapshot);


                        for (DataSnapshot child: dataSnapshot.getChildren()) {

                            AccessCode access=child.getValue(AccessCode.class);
                            System.out.println("Access Code Parts:"+ access.getCode());
                            code.add(access.getCode());

                        }
                        for(String codeCheck: code){
                            System.out.println("Access Codes: "+codeCheck);
                            if(input.getText().toString().equals(codeCheck)){
                                System.out.println("Correct Access Code");
                               // return;
                                Intent intent = new Intent(Login.this, admincreate.class);
                        ...
                        System.out.println("Error Wrong Access Code");
                        AlertAccess();
    ...
    public void AlertAccess(){
        final CharSequence[] items = {"Close"};
        AlertDialog.Builder builder = new AlertDialog.Builder(Login.this);
        builder.setTitle("Wrong Access Code");
...
//        ((GlobalVariables) this.getApplication()).setEmailAddress(email);
        ...
        GlobalVariables gs = (GlobalVariables) Login.this.getApplicationContext();
        gs.setUsername(user);
